...
function finiteNumberOrNull(value: string): number | null {
  const parsed = Number(value);
...
export function EquipmentScreen() {
  ...
  const [readingParameterType, setReadingParameterType] = useState<EquipmentDashboard["readings"][number]["parameterType"]>("temperature");
  const [readingValue, setReadingValue] = useState("21.5");
  const [readingUnit, setReadingUnit] = useState("C");
  const [readingMin, setReadingMin] = useState("18");
  const [readingMax, setReadingMax] = useState("25");
  const [cleaningExpiry, setCleaningExpiry] = useState("2026-12-27T08:00");
  const [downtimeReason, setDowntimeReason] = useState("changeover_adjustment");
  ...
  async function submitPreUseCheck(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!auth.session || !selectedEquipment) {
      return;
    }
    const templateId = selectedEquipment.equipmentType === "scale" ? "preuse-scale-weigh" : selectedEquipment.equipmentType === "bottling" ? "preuse-bottling-filler" : `preuse-${selectedEquipment.equipmentType}`;
    const response = await completeEquipmentPreUseCheck(auth.session.accessToken, {
      equipmentId: selectedEquipment.id,
      templateId,
      routingOperationId: selectedEquipment.equipmentType === "bottling" ? "routing-op-fill" : null,
      productionOrderId: "po-001",
      ebrExecutionId: "ebr-exec-lm-bot-001",
      checkedItems:
        selectedEquipment.equipmentType === "scale"
          ? [
              { itemId: "level", label: "Scale level and stable", passed: true, required: true },
              { itemId: "zero", label: "Zero/tare verified", passed: true, required: true }
            ]
          : [
      ...
      completedAt: new Date().toISOString(),
      notes: "Pre-use inspection completed from equipment checklist."
    });
    setDashboard(response.dashboard);
    showToast({ title: "Pre-use check complete", description: `${selectedEquipment.code} passed required readiness checks.` });
  }

  async function submitCleaning(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!auth.session || !selectedEquipment) {
      return;
    }
    const response = await recordEquipmentCleaning(auth.session.accessToken, {
      equipmentId: selectedEquipment.id,
      cleaningType: "changeover",
      status: "clean",
      cleanedAt: new Date().toISOString(),
      expiresAt: dateTimeLocalToIso(cleaningExpiry),
      productionOrderId: "po-001",
      ebrExecutionId: "ebr-exec-lm-bot-001",
      procedureId: "SOP-CLEAN-EQUIPMENT",
      notes: "Sanitation log recorded before equipment assignment."
    });
    setDashboard(response.dashboard);
    showToast({ title: "Cleaning logged", description: `${selectedEquipment.code} sanitation status is clean.` });
  }
  ...
  async function submitReading(event: FormEvent<HTMLFormElement>) {
    ...
    const value = finiteNumberOrNull(readingValue);
    if (value === null) {
      setError("Process reading value must be numeric.");
      return;
    }
    const response = await recordEquipmentReading(auth.session.accessToken, {
      equipmentId: selectedEquipment.id,
      productionOrderId: "po-001",
      processingBatchId: "batch-lm-bot-001",
      ebrExecutionId: "ebr-exec-lm-bot-001",
      routingOperationId: selectedEquipment.equipmentType === "bottling" ? "routing-op-fill" : null,
      parameterType: readingParameterType,
      parameterName: readingParameterType === "custom" ? "Custom reading" : readingParameterType,
      value,
      unit: readingUnit,
      source: "manual",
      recordedAt: new Date().toISOString(),
      minValue: finiteNumberOrNull(readingMin),
      maxValue: finiteNumberOrNull(readingMax),
      createDeviationOnOutOfLimit: true,
      createQualityEventOnOutOfLimit: true
    });
    setDashboard(response.dashboard);
    showToast({ title: "Process reading recorded", description: `${selectedEquipment.code} ${readingParameterType} captured.` });
  }

  async function submitDowntime(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!auth.session || !selectedEquipment) {
      return;
    }
    const response = await recordEquipmentDowntime(auth.session.accessToken, {
      equipmentId: selectedEquipment.id,
      reasonCode: downtimeReason,
      startedAt: new Date().toISOString(),
      productionOrderId: "po-001",
      routingOperationId: selectedEquipment.equipmentType === "bottling" ? "routing-op-fill" : null,
      notes: "Downtime captured from equipment board."
    });
    setDashboard(response.dashboard);
    showToast({ title: "Downtime recorded", description: `${selectedEquipment.code} is flagged for downtime review.` });
  }
      ...
      <Tabs
        tabs={[
          ...
          },
          {
            ...
            content: (
              <div className="split-panel">
                <form className="table-panel compact-form-grid" onSubmit={submitPreUseCheck}>
                  <div className="panel-heading">
                    <h3>Pre-use checklist</h3>
                    <Badge tone="success"><ClipboardList aria-hidden="true" size={16} /> Readiness gate</Badge>
                  </div>
                  <EquipmentSelect equipment={equipment} value={selectedEquipmentId} onChange={setSelectedEquipmentId} />
                  <div className="checkbox-grid full-span">
                    <label><input checked readOnly type="checkbox" /> Required checks passed</label>
                    <label><input checked readOnly type="checkbox" /> Critical operation gate</label>
                  </div>
                  <Button type="submit" disabled={!selectedEquipment}>
                    <CheckCircle2 aria-hidden="true" size={18} />
                ...
                <RecordList
                  emptyText="No pre-use checks recorded."
                  rows={(dashboard?.preUseChecks ?? []).map((check) => ({
                    id: check.id,
                    equipment: equipmentById.get(check.equipmentId)?.code ?? check.equipmentId,
                    primary: check.templateId,
                    secondary: check.status,
                    when: check.completedAt
                  }))}
          ...
          },
          {
            ...
            content: (
              <div className="split-panel">
                <form className="table-panel compact-form-grid" onSubmit={submitCleaning}>
                  <div className="panel-heading">
                    <h3>Cleaning log</h3>
                    <Badge tone="success"><CheckCircle2 aria-hidden="true" size={16} /> Sanitation status</Badge>
                  </div>
                  <EquipmentSelect equipment={equipment} value={selectedEquipmentId} onChange={setSelectedEquipmentId} />
                  <Input label="Clean until" type="datetime-local" value={cleaningExpiry} onChange={(event) => setCleaningExpiry(event.target.value)} />
                  <Button type="submit" disabled={!selectedEquipment}>
                ...
                <RecordList
                  emptyText="No cleaning logs recorded."
                  rows={(dashboard?.cleaningLogs ?? []).map((log) => ({
                    id: log.id,
                    equipment: equipmentById.get(log.equipmentId)?.code ?? log.equipmentId,
                    primary: `${log.cleaningType.replaceAll("_", " ")} / ${log.status}`,
                    secondary: log.expiresAt ? `expires ${formatDateTime(new Date(log.expiresAt))}` : "no expiry",
                    when: log.cleanedAt
                  }))}
                  title="Sanitation history"
                />
              </div>
            )
          },
          {
            ...
            content: (
              <div className="split-panel">
                <form className="table-panel compact-form-grid" onSubmit={submitReading}>
                  <div className="panel-heading">
                    <h3>Process readings</h3>
                    <Badge tone="info"><Zap aria-hidden="true" size={16} /> Manual/mock PLC</Badge>
                  </div>
                  <EquipmentSelect equipment={equipment} value={selectedEquipmentId} onChange={setSelectedEquipmentId} />
                  <label className="select-field">
                    <span>Parameter</span>
                    <select value={readingParameterType} onChange={(event) => setReadingParameterType(event.target.value as typeof readingParameterType)}>
                      {["temperature", "humidity", "pressure", "rpm", "time", "ph", "brix", "moisture", "custom"].map((parameter) => (
                        <option key={parameter} value={parameter}>{parameter}</option>
                      ))}
                    </select>
                  </label>
                  <Input label="Reading value" type="number" step="0.01" value={readingValue} onChange={(event) => setReadingValue(event.target.value)} />
                  <Input label="Unit" value={readingUnit} onChange={(event) => setReadingUnit(event.target.value)} />
                  <Input label="Minimum limit" type="number" step="0.01" value={readingMin} onChange={(event) => setReadingMin(event.target.value)} />
                  <Input label="Maximum limit" type="number" step="0.01" value={readingMax} onChange={(event) => setReadingMax(event.target.value)} />
                  <Button type="submit" disabled={!selectedEquipment || !readingUnit.trim()}>
                    <Save aria-hidden="true" size={18} />
                    Record process reading
                  </Button>
                </form>
                <RecordList
                  emptyText="No process readings recorded."
                  rows={(dashboard?.readings ?? []).map((reading) => ({
                    id: reading.id,
                    equipment: equipmentById.get(reading.equipmentId)?.code ?? reading.equipmentId,
                    primary: `${reading.parameterName ?? reading.parameterType}: ${reading.value} ${reading.unit}`,
                    secondary: reading.limitStatus.replaceAll("_", " "),
                    when: reading.recordedAt
                  }))}
                  title="Historian stream"
                />
              </div>
            )
          },
          {
            ...
            content: (
              <div className="split-panel">
                <form className="table-panel compact-form-grid" onSubmit={submitDowntime}>
                  <div className="panel-heading">
                    <h3>Downtime board</h3>
                    <Badge tone="warning"><AlertTriangle aria-hidden="true" size={16} /> Event stream</Badge>
                  </div>
                  <EquipmentSelect equipment={equipment} value={selectedEquipmentId} onChange={setSelectedEquipmentId} />
                  <Input label="Reason code" value={downtimeReason} onChange={(event) => setDowntimeReason(event.target.value)} />
                  <Button type="submit" disabled={!selectedEquipment || !downtimeReason.trim()}>
                ...
                <RecordList
                  ...
                  rows={(dashboard?.events ?? [])
                    .filter((event) => event.eventType === "downtime_recorded")
                    .map((event) => ({
                      id: event.id,
                      equipment: equipmentById.get(event.equipmentId)?.code ?? event.equipmentId,
                      primary: event.title,
...
function RecordList({
  ...
  emptyText: string;
  rows: Array<{ id: string; equipment: string; primary: string; secondary: string; when: string }>;
  title: string;
}) {
  const { formatDateTime } = useI18n();
  return (
    <div className="table-panel">
      <div className="panel-heading">
        <h3>{title}</h3>
        <Badge tone="info"><History aria-hidden="true" size={16} /> {rows.length}</Badge>
      </div>
      <table className="list-table">
        <thead><tr><th>Equipment</th><th>Record</th><th>When</th></tr></thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.id}>
              <td>{row.equipment}</td>
              ...
              <td>{formatDateTime(new Date(row.when))}</td>
            </tr>
          ))}
          {rows.length === 0 ? <tr><td colSpan={3}>{emptyText}</td></tr> : null}
        </tbody>
      </table>
    </div>
  );
