...
function AppShell() {
  ...
  const currentGroupId = activeGroupId(pathname, desktopNavGroups);
...
type PermissionLevel = "none" | "view" | "use" | "manage" | "approve";
...
const rolePermissionProfiles: Record<string, Record<string, PermissionLevel>> = {
...
function permissionForRole(roleCode: string, moduleId: string): PermissionLevel {
  return rolePermissionProfiles[roleCode]?.[moduleId] ?? "none";
}

function permissionTone(level: PermissionLevel): "neutral" | "success" | "warning" | "info" {
  if (level === "approve" || level === "manage") return "success";
  if (level === "use") return "info";
  if (level === "view") return "neutral";
  return "warning";
}

function effectivePermission(roles: Role[], moduleId: string): PermissionLevel {
  return roles.reduce<PermissionLevel>((current, role) => {
          ...
          tabs={[
            {
              ...
              content: <PermissionMatrix roles={roles} locations={locations} />
            },
            {
              id: "roles",
              label: "Role cards",
